drop policy "Users can manage and view their own participation" on "public"."conversation_participants";

drop policy "Users can view their own conversations" on "public"."conversations";

drop policy "Allow authenticated users to insert their own attachments" on "public"."direct_message_attachments";

drop policy "Allow conversation participants to view attachments" on "public"."direct_message_attachments";

drop policy "Allow users to delete their own attachments" on "public"."direct_message_attachments";

drop policy "Allow access to reactions within a conversation" on "public"."direct_message_reactions";

drop policy "Allow users to manage their own reactions" on "public"."direct_message_reactions";

drop policy "Allow participant to update message status" on "public"."direct_messages";

drop policy "Users can read messages in their conversations" on "public"."direct_messages";

drop policy "Users can send messages in their conversations" on "public"."direct_messages";

alter table "public"."location_aliases" drop constraint "location_aliases_location_id_fkey";

drop view if exists "public"."inbox_conversations";

alter table "public"."location_aliases" drop constraint "location_aliases_pkey";

drop index if exists "public"."location_aliases_pkey";

drop table "public"."location_aliases";
...
create policy "allow_admin_insert_on_announcements"
...
create policy "Users can view participants of their own conversations"
on "public"."conversation_participants"
as permissive
for select
to public
using ((EXISTS ( SELECT 1
   FROM conversation_participants my_entry
  WHERE ((my_entry.conversation_id = conversation_participants.conversation_id) AND (my_entry.user_id = auth.uid())))));
...
create policy "Message sender can delete their message's attachments"
...
using ((EXISTS ( SELECT 1
  ...
  WHERE ((dm.id = direct_message_attachments.message_id) AND (dm.sender_id = auth.uid())))));
...
create policy "Participants can see reactions in their conversation"
...
using ((EXISTS ( SELECT 1
   FROM (direct_messages dm
     JOIN conversation_participants cp ON ((dm.conversation_id = cp.conversation_id)))
  WHERE ((dm.id = direct_message_reactions.message_id) AND (cp.user_id = auth.uid())))));
